...
const Card = ({ id, title, type, img, views = 0, likes = 0, hidden, isOwner, onClick }) => {
    ...
    if (serviceData) {
        ...
        return (
            <div onClick={() => (onClick && (isOwner || !hidden)) && onClick(id)} className='result-preview-card' style={{ backgroundImage: `url(${img || serviceData.prevImg})`, cursor: onClick && (!hidden || isOwner) ? 'pointer' : 'unset', opacity: (hidden && !isOwner) ? '0.6' : '1' }}>
